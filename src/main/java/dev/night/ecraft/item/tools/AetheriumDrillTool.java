...
public class AetheriumDrillTool extends MultiMiningTool {
    private static final int maxEnergy = 900;
    private static final int fuelPerMagicCrystal = 150;
    private static final int rangeBonus = 5;

    public AetheriumDrillTool() {
        super(
        ...
        return AttributeModifiersComponent.builder()
                .add(
                        ...
                        new EntityAttributeModifier(
                                BASE_ATTACK_DAMAGE_MODIFIER_ID, (double)(EWeapons.HOE_BASE_ATTACK + EToolMaterials.FLAWLESS_AETHERIUM.getAttackDamage()), EntityAttributeModifier.Operation.ADD_VALUE
                        ),
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        // Give player extended range if the drill beam mode is set right
        AetheriumDrillComponent component = stack.getOrDefault(EComponents.AETHERIUM_DRILL_COMPONENT, getDefaultComponent());
        if (entity instanceof ServerPlayerEntity serverPlayer && selected && component.drillBeamMode() == 0 && component.currentEnergy() > 0) {
            serverPlayer.addStatusEffect(
                    new StatusEffectInstance(
                            EStatusEffects.FOCUSED_DRILL_BEAM, 5, 0, false, false
                    )
            );
        }
    }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        tooltip.add(Text.translatable("item.ecraft.aetherium_laser_drill.tooltip.energy", energy, maxEnergy));
        tooltip.add(Text.translatable("item.ecraft.aetherium_laser_drill.tooltip.energy_refill", energy, maxEnergy));
        switch (mode) {
            case 0 -> tooltip.add(Text.translatable("item.ecraft.aetherium_laser_drill.tooltip.focus_beam"));
            case 1 -> tooltip.add(Text.translatable("item.ecraft.aetherium_laser_drill.tooltip.standard_beam"));
            case 2 -> tooltip.add(Text.translatable("item.ecraft.aetherium_laser_drill.tooltip.tunneling_beam"));
            case 3 -> tooltip.add(Text.translatable("item.ecraft.aetherium_laser_drill.tooltip.wide_beam"));
        }

        if (efficiencyLevel <= 0)
            tooltip.add(Text.translatable("item.ecraft.aetherium_laser_drill.tooltip.efficiency_upgrade"));
        else
            tooltip.add(Text.translatable("item.ecraft.aetherium_laser_drill.tooltip.efficiency_level", efficiencyLevel * 10 + "%"));
    }
    ...
    public boolean onStackClicked(ItemStack stack, Slot slot, ClickType clickType, PlayerEntity player) {
        if (clickType == ClickType.RIGHT) {
            if (slot.getStack().getItem().equals(EItems.MAGIC_CRYSTAL)) {
                ...
                return true;
            } else if (slot.getStack().getItem().equals(EItems.FLAWLESS_AETHERIUM_CRYSTAL)) {
                // Increase mining efficiency level
                AetheriumDrillComponent component = stack.getOrDefault(EComponents.AETHERIUM_DRILL_COMPONENT, getDefaultComponent());
                byte level = component.efficiencyUpgradeLevel();
                if (level >= 5)
                    return false;

                level++;
                slot.getStack().decrement(1);
                stack.set(EComponents.AETHERIUM_DRILL_COMPONENT, new AetheriumDrillComponent(component.currentEnergy(), component.drillBeamMode(), level));
                return true;
            }
        }
        return false;
    ...
    public boolean onClicked(ItemStack stack, ItemStack otherStack, Slot slot, ClickType clickType, PlayerEntity player, StackReference cursorStackReference) {
        if (clickType == ClickType.RIGHT) {
            if (otherStack.getItem().equals(EItems.MAGIC_CRYSTAL)) {
                ...
                stack.set(EComponents.AETHERIUM_DRILL_COMPONENT, new AetheriumDrillComponent(energy, component.drillBeamMode(), component.efficiencyUpgradeLevel()));
                return true;
            } else if (otherStack.getItem().equals(EItems.FLAWLESS_AETHERIUM_CRYSTAL)) {
                // Increase mining efficiency level
                AetheriumDrillComponent component = stack.getOrDefault(EComponents.AETHERIUM_DRILL_COMPONENT, getDefaultComponent());
                byte level = component.efficiencyUpgradeLevel();
                if (level >= 5)
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (world instanceof ServerWorld && hand.equals(Hand.MAIN_HAND)) {
            ...
            switch (drillBeamMode) {
                case 0 -> user.sendMessage(Text.translatable("item.ecraft.aetherium_laser_drill.focus_beam"), true);
                case 1 -> user.sendMessage(Text.translatable("item.ecraft.aetherium_laser_drill.standard_beam"), true);
                case 2 -> user.sendMessage(Text.translatable("item.ecraft.aetherium_laser_drill.tunneling_beam"), true);
                case 3 -> user.sendMessage(Text.translatable("item.ecraft.aetherium_laser_drill.wide_beam"), true);
            }
    ...
    public boolean postMine(ItemStack stack, World world, BlockState state, BlockPos pos, LivingEntity miner) {
        if (miner instanceof ServerPlayerEntity serverPlayer) {
            ...
            if (currentEnergy <= 0) {
                currentEnergy = 0;
                serverPlayer.sendMessage(Text.translatable("item.ecraft.aetherium_laser_drill.no_fuel"), true);
            }
