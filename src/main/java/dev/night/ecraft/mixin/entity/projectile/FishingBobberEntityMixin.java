package dev.night.ecraft.mixin.entity.projectile;
...
public abstract class FishingBobberEntityMixin extends ProjectileEntity implements FishingBobberEntityInterface {
    ...
    private void redirectedInitSpeed(FishingBobberEntity bobber, Vec3d originalVec3d, PlayerEntity thrower, World world, int luckOfTheSeaLevel, int lureLevel) {
        ...
        if (this.fishingRodStack != null && this.fishingRodStack.getItem() instanceof EModFishingRodItem) {
            ToolMaterial material = ((EModFishingRodItem)this.fishingRodStack.getItem()).getMaterial();
            int i = 1;
    ...
    private boolean redirectedFluidIsIn(FluidState fluidState, TagKey<Fluid> originalTag) {
        if (this.fishingRodStack != null && this.fishingRodStack.getItem() instanceof EModFishingRodItem && ((EModFishingRodItem)this.fishingRodStack.getItem()).getMaterial() == ToolMaterials.NETHERITE) {
    ...
    private float redirectedPlayerLuck(PlayerEntity player, ItemStack usedStack) {
        if (this.getWorld() instanceof ServerWorld && usedStack != null && usedStack.getItem() instanceof EModFishingRodItem) {
            ServerWorld serverWorld = (ServerWorld)this.getWorld();
            ToolMaterial material = ((EModFishingRodItem)usedStack.getItem()).getMaterial();
