package dev.night.ecraft.mixin.client.render.item;
...
public abstract class ItemRendererMixin implements SynchronousResourceReloader {
    ...
    private BakedModel modifiedRenderedModel(BakedModel model, ItemStack stack, ModelTransformationMode renderMode, boolean leftHanded, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        if (!stack.isEmpty()) {
            ...
            if ((item instanceof SpearItem) && renderModeBl) {
                String name = Registries.ITEM.getId(item).getPath();
                ModelIdentifier modelId = new ModelIdentifier(Identifier.of(SpearsAxesSwordsShieldsAndOtherTools.MOD_ID, name), "inventory");
                model = this.models.getModelManager().getModel(modelId);
                model.getTransformation().getTransformation(renderMode).apply(leftHanded, matrices);

                Ecraft.LOGGER.info("modifiedRenderModel " + name  + "\n" + modelId.toString());
            }
        }
        return model;
    }
    ...
    private BakedModel modifiedBakedModel(BakedModel originalBakedModel, ItemStack stack, @Nullable World world, @Nullable LivingEntity entity, int seed) {
        if (stack.getItem() instanceof SpearItem) {
            ...
            ModelIdentifier inHandModelId = new ModelIdentifier(Identifier.of(SpearsAxesSwordsShieldsAndOtherTools.MOD_ID, name + "_in_hand"), "inventory");

            Ecraft.LOGGER.info("modifiedRenderModel2 " + name + "\n" + inHandModelId.toString());
